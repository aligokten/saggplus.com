...
export default function ServicesList({ services }: { services: ServiceRecord[] }) {
  ...
  async function persistOrder(next: ServiceRecord[]) {
    setItems(next);
    setBusy(true);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-muted">{items.length} hizmet</p>
        </div>
        <Link
      ...
      <div className="flex flex-col gap-3">
        {items.length === 0 && (
          <p className="text-sm text-ink-muted">Henüz hizmet eklenmedi.</p>
        )}
        {items.map((service, i) => {
          const Icon = SERVICE_ICONS[service.icon] ?? SERVICE_ICONS.compass;
          return (
            <div key={service.id} className="flex items-center gap-4 rounded-2xl glass p-4">
